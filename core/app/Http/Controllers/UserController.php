...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $accountType = Plan::where("id", auth()->user()->plan_id)->first(["name"]);
        $data['accountType']        = $accountType==null ? '<p style="color: red;">(Free Account) Please Upgrade Your Account Package, Otherwise Your Account Will Be Deactivated in 24 Hours  </p>' : $accountType->name;
        $data['sponsoredBy']        = User::where("id", auth()->user()->ref_id)->first(["firstname","lastname"]);
        $data['adRevenue']        = Adrevenue::where("user_id", auth()->user()->id)->sum("ad_revenue");
        $data['withdraw_bonuses']        = withdraw_bonus::where("user_id", auth()->user()->id)->sum("withdrawed_bonus");
    ...
    public function withdrawStore(Request $request)
    {
        ...
        $max_withdraw=getAmount(auth()->user()->balance) +$adRevenue +auth()->user()->userExtra->bv_left +auth()->user()->userExtra->bv_right +getAmount(auth()->user()->total_ref_com)+getAmount(auth()->user()->total_binary_com)-$withdrawed_bonus_sum;
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        $adRevenue       = Adrevenue::where("user_id", auth()->user()->id)->sum("ad_revenue");
        $user_blance=getAmount(auth()->user()->balance) ;
        $user_earn=$adRevenue +auth()->user()->userExtra->bv_left +auth()->user()->userExtra->bv_right +getAmount(auth()->user()->total_ref_com)+getAmount(auth()->user()->total_binary_com);
        $withdrawal_request=$withdraw->amount;
        $withdrawed_bonus_sum=   withdraw_bonus::where("user_id", auth()->user()->id)->sum("withdrawed_bonus");;
        $remaing_my_cash_balance=$user_blance+$user_earn-$withdrawal_request-$withdrawed_bonus_sum;

        if($user_blance <= $withdrawal_request){
            $user->balance  =0;
        }else{
            $user->balance  -=  $withdraw->amount;
        }
        $total_withdrawed_from_blance=$user_blance-$withdrawal_request;
        if($total_withdrawed_from_blance <  0){
            $obj=new withdraw_bonus();
            $obj->user_id=auth()->user()->id;
            $obj->withdrawal_id=$withdraw->id;
            $obj->withdrawed_bonus=-$total_withdrawed_from_blance;
            $obj->save();
